...
export const shareHabit = catchAsync(async (req, res) => {
  let shared = await sharedHabitService.shareHabit(
    req.user._id,
    req.body.habitId,
  );
...
export const joinByInviteCode = catchAsync(async (req, res) => {
  const shared = await sharedHabitService.joinByInviteCode(
    req.user._id,
    req.body.inviteCode,
  );
...
export const inviteMember = catchAsync(async (req, res) => {
  ...
  const { shared, emailSent, emailError } =
    await sharedHabitService.inviteMember(req.user._id, habitId, email, role);
...
export const respondToInvite = catchAsync(async (req, res) => {
  ...
  const shared = await sharedHabitService.respondToInvite(
    req.user._id,
    habitId,
    accept,
  );
...
export const leaveHabit = catchAsync(async (req, res) => {
  const result = await sharedHabitService.leaveSharedHabit(
    req.user._id,
    req.params.habitId,
  );
...
export const getSharedWithMe = catchAsync(async (req, res) => {
  const sharedHabits = await sharedHabitService.getSharedHabitsForUser(
    req.user._id,
  );
...
export const getSharingInfo = catchAsync(async (req, res) => {
  const result = await sharedHabitService.getSharingInfo(
    req.user._id,
    req.params.habitId,
  );
...
export const regenerateInviteCode = catchAsync(async (req, res) => {
  const shared = await sharedHabitService.regenerateInviteCode(
    req.user._id,
    req.params.habitId,
  );
...
export const unshareHabit = catchAsync(async (req, res) => {
  const result = await sharedHabitService.unshareHabit(
    req.user._id,
    req.params.habitId,
  );
...
export const getSharedByMe = catchAsync(async (req, res) => {
  const sharedHabits = await sharedHabitService.getHabitsSharedByUser(
    req.user._id,
  );
...
export const getInvitePreview = catchAsync(async (req, res) => {
  const preview = await sharedHabitService.getInvitePreview(
